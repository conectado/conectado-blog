...
impl Server {
    pub async fn new() -> Server {
        ...
        tokio::spawn(central_dispatcher(tx.clone(), rx));
        Server { tx, listener }
    }
    ...
    async fn handle_connection(&self, socket: TcpStream) {
        ...
        loop {
            ...
            self.tx.send(Message::Send(dest, m, id)).await.unwrap();
        }
    }
}

async fn central_dispatcher(tx: mpsc::Sender<Message>, mut rx: mpsc::Receiver<Message>) {
    ...
    while let Some(msg) = rx.recv().await {
        match msg {
            Message::New(id, connection) => {
                ...
                let handle = tokio::spawn(client_dispatcher(connection, rx, error_tx.clone()))
                    .abort_handle();
